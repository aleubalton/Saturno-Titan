...
public class ServicioResource {
    ...
    @Timed
    public ResponseEntity<List<ServicioDTO>> getAllServicios(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Servicios");
        Page<ServicioDTO> page;
        if (eagerload) {
            page = servicioService.findAllWithEagerRelationships(pageable);
        } else {
            page = servicioService.findAll(pageable);
        }
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, String.format("/api/servicios?eagerload=%b", eagerload));
